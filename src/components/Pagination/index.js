...
const Pagination = ({ totalPage, currentPage, onChangeCurrentPage }) => {
	...
	return (
		<StyledPagination>
			<BoostrapPagination.Item disabled={currentPage === 1} onClick={prevPageHandler}>
				Previous
			</BoostrapPagination.Item>
			{paginationItems.map((item, index) => {
				const isActive = currentPage === index + 1;
				return (
					<BoostrapPagination.Item key={index} active={isActive} onClick={onChangePageHandler(isActive, index + 1)}>
						{index + 1}
					</BoostrapPagination.Item>
				);
			})}
			<BoostrapPagination.Item disabled={currentPage === totalPage} onClick={nextPageHandler}>
				Next
			</BoostrapPagination.Item>
		</StyledPagination>
	);
